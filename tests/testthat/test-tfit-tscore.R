...
test_that("Check the Values for model tscores; match dimension and value", {
  ...
  tscore_frame_maSigPro <- tfit$t.score
  ...
  tscore_frame_scMaSigPro <- as.data.frame(showTscores(scmp.obj, view = F))

  ######################################################
  # Transfer column and row names for testing
  colnames(tscore_frame_scMaSigPro) <- colnames(tscore_frame_maSigPro)
  ######################################################

  # Step-6: First, check that the matrices have the same dimensions
  expect_equal(dim(tscore_frame_scMaSigPro), dim(tscore_frame_maSigPro))

  # Step-7: Next, check that all elements in the matrices are the same
  expect_equal(tscore_frame_scMaSigPro, tscore_frame_maSigPro)
})
