...
test_that("Check-'tfit$coefficients'", {
  # Step-1: Load Data
  data("data.abiotic")
  ...
  # Check-sol
  # Poly-order-2
  expect_identical(showCoeff(test.scmp.2), expected = tstep_2$coefficients)
  # Poly-order-3
  expect_identical(showCoeff(test.scmp.3), expected = tstep_3$coefficients)
  # Poly-order-4
  expect_identical(showCoeff(test.scmp.4), expected = tstep_4$coefficients)
})
