...
test_that("Check-'tstep$SOL' Reproducibility: Match, Dimension, Name and Value", {
    ...
    test.scmp.4 <- sc.p.vector(test.scmp.4, min.obs = 20,
                               offset = F, parallel = T,
                               family = gaussian(), verbose = F)
    ...
    # Extract sigs
    test.scmp.2 <- sc.get.siggenes(test.scmp.2, rsq = 0.6, vars = "groups")
    test.scmp.3 <- sc.get.siggenes(test.scmp.3, rsq = 0.6, vars = "groups")
    test.scmp.4 <- sc.get.siggenes(test.scmp.4, rsq = 0.6, vars = "groups")
    
    # Check
    # Poly-order-2
    expect_equal(expected = sigs2$summary,
                 object = test.scmp.2@sig.genes@summary)
    # Poly-order-3
    expect_equal(expected = sigs3$summary,
                 object = test.scmp.3@sig.genes@summary)
    # Poly-order-4
    expect_equal(expected = sigs4$summary,
