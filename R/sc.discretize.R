...
#'
#' @description
#' `discretize()` discretizes a continuous time series column into bins
...
#' @param per_path A logical. When set to "individual", the bins are calculated
...
sc.discretize <- function(scmpObject,
  ...
  if (verbose) {
    ...
    message(paste("Initiating binning by:", per_path, "method."))
  }
  ...
  if(per_path){
      ...
      discrete.list <- lapply(avail.paths, function(path, design.frame = cell_metadata,
          ...
          print(sum(bin_table[[bin_size_colname]]))
          if (hom_bin) {
              bin_table <- homogenize.bins(
                  df = bin_table,
                  l_bound_col = lbound,
                  u_bound_col = ubound,
      ...
      if (verbose) {
          message(paste(
              "Finally,", length_n, "time points has been compressed to", nrow(bin_table), "bins"
          ))
      }
  ...
  scmpObject@addParams@per_path <- per_path
