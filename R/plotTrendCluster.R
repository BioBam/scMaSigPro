...
plotTrendCluster <- function(scmpObj, 
    ...
    # Extarct cluster set
    gene_cluster_vector <- as.vector(unlist(scmpObj@sig.genes@feature.clusters))
    
    # Create df
    gene.cluster.df <- data.frame(gene =  gene_set_vector,
                                  clus = gene_cluster_vector)
    colnames(gene.cluster.df) <- c(feature_id, scmp_clusters)
    
    # # Extract data based on 'plot'
    # if (plot == "counts") {
    #     # Extract bulk counts
    #     data_matrix <- as.matrix(showSigProf(scmpObj, includeInflu = includeInflu))
    # } else if (plot == "coeff") {
    #     # Extract coefficients
    #     data_matrix <- as.matrix(showCoeff(scmpObj, includeInflu = includeInflu))
    # }
    # 
    # # Select
    # data_matrix <- data_matrix[gene.cluster.df[[feature_id]],,drop =FALSE]
    # 
    # # Perform operation as per the sc.cluster.trend()
    # # Get variables
    # fill_dim = scmpObj@param@cluster.fill.dim
    # fill_na_by = scmpObj@param@cluster.fill.na
    # 
    # # Check fill by
    # if (fill_dim == "row") {
    #     use_dim <- 1
    ...
    # if (fill_na_by == "mean") {
    #     data_matrix <- apply(data_matrix, use_dim, function(x) {
    ...
    collapsed.df <- lapply(cluster.data.list, function(clus_i.list,
        ...
        line.df <- line.df %>% 
            ...
            summarize(y_axis = mean(y_axis, na.rm = TRUE), .groups = 'drop')
        point.df <- point.df %>% 
            group_by(x_axis, group) %>% 
            summarize(y_axis = mean(y_axis, na.rm = TRUE),  .groups = 'drop')
        curve.df <- curve.df %>% 
    ...
    lines_combined <- do.call(rbind, lines_list)
    curves_combined <- do.call(rbind, curves_list)
    points_combined <- do.call(rbind, points_list)
