#' Detect Genes with Isoforms Expressing Differently in Time Course Experiments
#'
#' The function `IsoModel` carries out a model comparison for each gene to pinpoint genes with varying trends in time course experiments, and implements maSigPro to the Isoforms associated with selected genes.
#'
#' @title IsoModel
#'
#' @param data A matrix holding isoform expression data. Isoforms should be in rows and experimental conditions in columns.
#' @param gen A vector with the name of the gene each isoform is tied to.
#' @param design The design matrix for the regression fit, like that generated by the `make.design.matrix` function. Default is NULL.
#' @param Q The significance level. Default is 0.05.
#' @param min.obs Cases having less than this count of true numerical values will be removed from the analysis. The minimum value to estimate the model is (degree+1)xGroups+1. Default is 6.
#' @param minorFoldfilter Fold expression difference between minor isoforms and the most expressed isoform to remove minor isoforms from the analysis. Default is NULL.
#' @param counts A logical indicating whether your data are counts. Default is FALSE.
#' @param family The distribution function to be utilized in the glm model. Must be specified as a function: gaussian(), poisson(), negative.binomial(theta) etc. If NULL, family will be negative.binomial(theta) when counts=TRUE or gaussian() when counts=FALSE.
#' @param theta Theta parameter for negative.binomial family. Default is 10.
#' @param epsilon Argument to pass to `glm.control`, convergence tolerance in the iterative process to estimate de glm model. Default is 1e-05.
#'
#' @details
#' The vectors `rownames(design)` and `colnames(data)` must be identical and represent experimental condition names.
#' The `rownames(data)` should contain unique isoform IDs.
#' The `colnames(design)` are the given names for the variables in the regression model.
#'
#' @return A list containing:
#' * `data`: The input data matrix used in the following steps.
#' * `gen`: The input gen vector used in the following steps.
#' * `design`: The input design matrix used in the following steps.
