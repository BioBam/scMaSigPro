...
#' @param design design matrix for the regression fit such as that generated by the \code{\link{make.design.matrix}} function.
...
sc.p.vector <- function(data, design, Q = 0.05, MT.adjust = "BH", min.obs = 6, counts = FALSE, family = NULL, theta = 10, epsilon = 0.00001, item = "gene") {
  ...
  # Convert 'data' to matrix and select relevant columns based on 'design' rows
  dat <- as.matrix(data)
  ...
  # Get dimensions for the input data
  g <- dim(dat)[1]
  n <- dim(dat)[2]
  p <- dim(dis)[2]
