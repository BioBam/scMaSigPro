...
extract_monocle3_components <- function(cds, reduction_method = "umap", verbose = TRUE) {
  ...
  if (verbose) {
    ...
    message(paste("Number of potential paths identified are", paste(length(endpoints))))
  }
  ...
  cell.meta[rownames(cell.meta) %in% c(rownames(y_to_cells[y_to_cells$Y %in% root, ])), "PrincipalPoints"] <- "root"
  ...
  cell.meta$Path <- convert_to_path(cell.meta[["PrincipalPoints"]])
