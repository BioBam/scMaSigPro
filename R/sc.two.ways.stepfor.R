"sc.two.ways.stepfor" <-
  function(y = y, d = d, alfa = 0.05, family = gaussian(), epsilon = 0.00001, useOffset, useWeight, weights = useWeight) {
    pval <- NULL
    design <- NULL
    j <- 1
    ...
    while (min <= alfa) {
      ...
      result2 <- summary(glm(y ~ ., data = design, family = family, epsilon = epsilon, offset = useOffset, weights = useWeight))$coefficients[
        ,
        4
      ]
