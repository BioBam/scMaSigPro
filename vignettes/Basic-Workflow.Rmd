...
FigureA <- sc.PlotGroups(scmp.ob, "Gene9", logs = T, logType = "log")
FigureB <- sc.PlotGroups(scmp.ob, "Gene95", logs = T, logType = "log")
FigureC <- sc.PlotGroups(scmp.ob, "Gene10", logs = T, logType = "log")
FigureD <- sc.PlotGroups(scmp.ob, "Gene92", logs = T, logType = "log")
...
### Cluster Trends
To discern general trends among the genes, we can cluster them together using the `sc.cluster.features()`. By default, this function employs hierarchical clustering (hclust) and we will divide the genes into 4 clusters.
```{r, "Cluster Trends"}
