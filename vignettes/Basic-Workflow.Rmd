...
scmp.ob <- sc.p.vector(scmp.ob, offset = TRUE, Q = 0.05,
...
Now that we have the refined models, we can further explore that which of gene are significant diferrences with pseudotime, among paths and both. To do that we will make use of the `sc.get.siggenes()`. We try to select only the models with relatively high $R^2$, as we are modeling simple linear relationships.
```{r}
scmp.ob <- sc.get.siggenes(scmpObj = scmp.ob,
                rsq = 0.7, 
                vars = "groups")
```

The `vars` parameter which is set to the "groups" will add the genes that are having $R^2$ >= 0.8 to the object. We can explpre the number of genes per groupo using the upsetplot as follows,
```{r}
sc.path.intersection(scmp.ob)
```

```{r}
sc.PlotGroups(scmp.ob, "Gene9", logs = T, logType = "log")
sc.PlotGroups(scmp.ob, "Gene95", logs = T, logType = "log")
sc.PlotGroups(scmp.ob, "Gene10", logs = T, logType = "log")
sc.PlotGroups(scmp.ob, "Gene92", logs = T, logType = "log")
```
