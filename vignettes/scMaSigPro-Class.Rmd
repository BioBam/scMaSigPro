...
## Dataset
Here, we will use `scmp_ob` that already has all the data inside it. The `scmp_ob`
...
# Load example data
data("scmp_ob_complete", package = "scMaSigPro")
```

As the object `scmp_ob` is already processed, we can type the name of the object
...
The `scmp_ob` object has two slots, `Sparse` and `Dense` both which are of 
...
two are generic methods and have similary functionality to the `assays()` from the 
...
eDense(scmp_ob)[c(1:3), c(1:3)]

# Get the Sparse counts
eSparse(scmp_ob)[c(1:3), c(1:3)]
```
...
cDense(scmp_ob)[c(1:3), ]

# Get cell level metadata
cSparse(scmp_ob)[c(1:3), c(1:3)]
```
